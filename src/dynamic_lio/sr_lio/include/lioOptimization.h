...
private:
    ...
    ros::Publisher pub_static_points_sampled;
    ros::Publisher pub_dynamic_points_sampled;
    ...
    voxelHashMap voxel_map_true;
    std::vector<point3D> points_undecided;
    std::vector<globalPoint*> points_vec_true;
    ...
    // loop closing
    ros::Publisher pub_loop_map;
    pcl::PointCloud<pcl::PointXYZI>::Ptr loop_map;

    void pubLocalMap(cloudFrame* p_frame);
    // loop closing
...
public:
    ...
    void process(std::vector<point3D> &const_frame, double timestamp_begin, double timestamp_offset);
    ...
    Neighborhood computeNeighborhoodDistribution(std::vector<globalPoint> &points);
    ...
    // map update
    void addPointToMap(voxelHashMap &map, point3D &point3d, double voxel_size, int max_num_points_in_voxel, double min_distance_points, int min_num_points, cloudFrame* p_frame);
    ...
    void publishCurrentSampledCloud(cloudFrame *p_frame, bool publish_dynamic);
    ...
    void addPointToPcl(pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_points, globalPoint& point);
    ...
    void addPointToTrueMap(voxelHashMap &map, point3D &point3d, double voxel_size, int max_num_points_in_voxel, double min_distance_points, int min_num_points);

    void decidePoints(const icpOptions &cur_icp_options, cloudFrame *p_frame);

    void addDecidedPointsToTrueMap(double voxel_size, int max_num_points_in_voxel, double min_distance_points, int min_num_points = 0);

    void detectDynamicLabel(const icpOptions &cur_icp_options, voxelHashMap &voxel_map_temp, cloudFrame *p_frame);
    // remove dynamic points

...
class numType
{
  public:
    ...
    static Eigen::Matrix<typename Derived::Scalar, 3, 2> derivativeS2(const Eigen::MatrixBase<Derived> &g_in)
    {
        ...
        const Scalar_t g_norm = g.norm();
        if (g_norm < static_cast<Scalar_t>(1e-6))
        {
            B_x << static_cast<Scalar_t>(1.0), static_cast<Scalar_t>(0.0),
                   static_cast<Scalar_t>(0.0), static_cast<Scalar_t>(1.0),
                   static_cast<Scalar_t>(0.0), static_cast<Scalar_t>(0.0);
            return B_x;
        }

        g /= g_norm;

        Scalar_t denom = static_cast<Scalar_t>(1.0) + g(2);
        if (std::abs(denom) < static_cast<Scalar_t>(1e-6))
            denom = denom >= static_cast<Scalar_t>(0.0) ? static_cast<Scalar_t>(1e-6) : static_cast<Scalar_t>(-1e-6);

        B_x(0, 0) = static_cast<Scalar_t>(1.0) - g(0) * g(0) / denom;
        B_x(0, 1) = - g(0) * g(1) / denom;
        B_x(1, 0) = B_x(0, 1);
        B_x(1, 1) = static_cast<Scalar_t>(1.0) - g(1) * g(1) / denom;
        B_x(2, 0) = - g(0);
        B_x(2, 1) = - g(1);

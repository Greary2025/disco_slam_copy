...
void cloudProcessing::velodyneHandler(const sensor_msgs::PointCloud2::ConstPtr &msg, double &dt_offset)
{
    ...
    const bool organized_cloud_matches_scan =
        static_cast<int>(msg->height) == N_SCANS && static_cast<int>(msg->width) == Horizon_SCANS;

    if ((!has_time_field || !has_ring_field) && organized_cloud_matches_scan)
    {
        pcl::PointCloud<pcl::PointXYZI> raw_cloud;
        pcl::fromROSMsg(*msg, raw_cloud);
        const int size = raw_cloud.points.size();
        ...
        if (!logged_missing_fields)
        {
            ROS_INFO("SR-LIO: point cloud missing ring/time. Using organized cloud layout (%u x %u) to synthesize both fields.",
                     msg->height, msg->width);
            logged_missing_fields = true;
        }

        given_ring = true;
        given_offset_time = true;

        const int msg_width = std::max(1, static_cast<int>(msg->width));
        const double synthetic_scan_period = std::max(delta_cut_time, 1.0 / std::max(1, SCAN_RATE) * 1000.0);
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            const int row_id = i / msg_width;
            const int col_id = i % msg_width;
            if (row_id < 0 || row_id >= N_SCANS || col_id < 0 || col_id >= Horizon_SCANS)
                continue;
            ...
            const double range = point_temp.raw_point.norm();
            if (range < blind)
                continue;
            if (range_mat.at<float>(row_id, col_id) != FLT_MAX)
                continue;

            point_temp.range = range;
            if (msg_width > 1)
                point_temp.relative_time = synthetic_scan_period * static_cast<double>(col_id) / static_cast<double>(msg_width - 1);
            else
                point_temp.relative_time = 0.0;
            point_temp.timestamp = msg->header.stamp.toSec() + point_temp.relative_time / 1000.0;
            point_temp.alpha_time = synthetic_scan_period > 1e-6 ? point_temp.relative_time / synthetic_scan_period : 0.0;

            range_mat.at<float>(row_id, col_id) = range;
            full_cloud[col_id + row_id * Horizon_SCANS] = point_temp;
        }

        delta_cut_time = synthetic_scan_period;
        dt_offset = synthetic_scan_period;
        return;
    }

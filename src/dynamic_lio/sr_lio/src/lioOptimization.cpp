...
lioOptimization::lioOptimization() : pnh("~")
{
	allocateMemory();
    ...
    pub_cloud_body = nh.advertise<sensor_msgs::PointCloud2>(robot_id + "/sr_lio/cloud_registered_current", 2);
    pub_cloud_world = nh.advertise<sensor_msgs::PointCloud2>(robot_id + "/sr_lio/cloud_global_map", 2);
    pub_odom = nh.advertise<nav_msgs::Odometry>(robot_id + "/sr_lio/odometry", 5);
    pub_path = nh.advertise<nav_msgs::Path>(robot_id + "/sr_lio/path", 5);
    pub_static_points_sampled = nh.advertise<sensor_msgs::PointCloud2>(robot_id + "/sr_lio/static_points_sampled", 5);
    pub_dynamic_points_sampled = nh.advertise<sensor_msgs::PointCloud2>(robot_id + "/sr_lio/dynamic_points_sampled", 5);
    ...
    path.header.frame_id = robot_id + "/sr_lio_init";
...
{	
    ...
    // common
    pnh.param<std::string>("robot_id", robot_id, std::string("jackal0"));
    pnh.param<std::string>("lidar_frame", lidar_frame, std::string("base_link"));
    pnh.param<std::string>("common/lidar_topic", lidar_topic, std::string("/points_raw"));
	pnh.param<std::string>("common/imu_topic", imu_topic, std::string("/imu_raw"));
    pnh.param<int>("common/point_filter_num", para_int, 1);  cloud_pro->setPointFilterNum(para_int);
    pnh.param<std::vector<double>>("common/gravity_acc", v_G, std::vector<double>());
    pnh.param<bool>("common/imu_has_gravity", imu_has_gravity, true);
    pnh.param<bool>("debug_output", debug_output, false);
    pnh.param<std::string>("output_path", output_path, "");

    // LiDAR parameter
    pnh.param<int>("lidar_parameter/lidar_type", para_int, AVIA);  cloud_pro->setLidarType(para_int);
    pnh.param<int>("lidar_parameter/N_SCANS", para_int, 16);  cloud_pro->setNumScans(para_int);
    pnh.param<int>("lidar_parameter/Horizon_SCANS", para_int, 1800);  cloud_pro->setHorizonScans(para_int);
    pnh.param<float>("lidar_parameter/ang_res_x", para_float, 0.2);  cloud_pro->setAngResX(para_float);
    pnh.param<float>("lidar_parameter/ang_res_y", para_float, 1.33);  cloud_pro->setAngResY(para_float);
    pnh.param<float>("lidar_parameter/ang_bottom", para_float, 30.67);  cloud_pro->setAngBottom(para_float);
    pnh.param<int>("lidar_parameter/ground_scan_id", para_int, 20);  cloud_pro->setGroundScanInd(para_int);

    pnh.param<int>("lidar_parameter/SCAN_RATE", para_int, 10);  cloud_pro->setScanRate(para_int);
    pnh.param<int>("lidar_parameter/time_unit", para_int, US);  cloud_pro->setTimeUnit(para_int);
    pnh.param<double>("lidar_parameter/blind", para_double, 0.01);  cloud_pro->setBlind(para_double);
    pnh.param<float>("lidar_parameter/det_range", det_range, 300.f);
    pnh.param<double>("lidar_parameter/fov_degree", fov_deg, 180);

    // IMU parameter
    pnh.param<double>("imu_parameter/acc_cov", para_double, 0.1);  eskf_pro->setAccCov(para_double);
    pnh.param<double>("imu_parameter/gyr_cov", para_double, 0.1);  eskf_pro->setGyrCov(para_double);
    pnh.param<double>("imu_parameter/b_acc_cov", para_double, 0.0001);  eskf_pro->setBiasAccCov(para_double);
    pnh.param<double>("imu_parameter/b_gyr_cov", para_double, 0.0001);  eskf_pro->setBiasGyrCov(para_double);

    pnh.param<bool>("imu_parameter/time_diff_enable", time_diff_enable, false);

    // extrinsic parameter
    pnh.param<bool>("extrinsic_parameter/extrinsic_enable", extrin_enable, true);
    pnh.param<std::vector<double>>("extrinsic_parameter/extrinsic_t", v_extrin_t, std::vector<double>());
    pnh.param<std::vector<double>>("extrinsic_parameter/extrinsic_R", v_extrin_R, std::vector<double>());

    // state estimation parameters
    pnh.param<double>("odometry_options/init_voxel_size", options.init_voxel_size, 0.2);
    pnh.param<double>("odometry_options/init_sample_voxel_size", options.init_sample_voxel_size, 1.0);
    pnh.param<int>("odometry_options/init_num_frames", options.init_num_frames, 20);
    pnh.param<double>("odometry_options/voxel_size", options.voxel_size, 0.5);
    pnh.param<double>("odometry_options/sample_voxel_size", options.sample_voxel_size, 1.5);
    pnh.param<double>("odometry_options/max_distance", options.max_distance, 100.0);
    pnh.param<int>("odometry_options/max_num_points_in_voxel", options.max_num_points_in_voxel, 20);
    pnh.param<double>("odometry_options/min_distance_points", options.min_distance_points, 0.1);
    pnh.param<double>("odometry_options/distance_error_threshold", options.distance_error_threshold, 5.0);

    pnh.param<std::string>("odometry_options/motion_compensation", str_temp, "CONSTANT_VELOCITY");
    ...
    pnh.param<std::string>("odometry_options/initialization", str_temp, "INIT_IMU");
    ...
    pnh.param<int>("icp_options/threshold_voxel_occupancy", options.optimize_options.threshold_voxel_occupancy, 1);
    pnh.param<double>("icp_options/size_voxel_map", options.optimize_options.size_voxel_map, 1.0);
    pnh.param<int>("icp_options/num_iters_icp", options.optimize_options.num_iters_icp, 5);
    pnh.param<int>("icp_options/min_number_neighbors", options.optimize_options.min_number_neighbors, 20);
    pnh.param<int>("icp_options/voxel_neighborhood", options.optimize_options.voxel_neighborhood, 1);
    pnh.param<double>("icp_options/power_planarity", options.optimize_options.power_planarity, 2.0);
    pnh.param<bool>("icp_options/estimate_normal_from_neighborhood", options.optimize_options.estimate_normal_from_neighborhood, true);
    pnh.param<int>("icp_options/max_number_neighbors", options.optimize_options.max_number_neighbors, 20);
    pnh.param<double>("icp_options/max_dist_to_plane_icp", options.optimize_options.max_dist_to_plane_icp, 0.3);
    pnh.param<double>("icp_options/threshold_orientation_norm", options.optimize_options.threshold_orientation_norm, 0.0001);
    pnh.param<double>("icp_options/threshold_translation_norm", options.optimize_options.threshold_translation_norm, 0.001);
    pnh.param<int>("icp_options/max_num_residuals", options.optimize_options.max_num_residuals, -1);
    pnh.param<int>("icp_options/min_num_residuals", options.optimize_options.min_num_residuals, 100);
    pnh.param<int>("icp_options/num_closest_neighbors", options.optimize_options.num_closest_neighbors, 1);
    pnh.param<double>("icp_options/weight_alpha", options.optimize_options.weight_alpha, 0.9);
    pnh.param<double>("icp_options/weight_neighborhood", options.optimize_options.weight_neighborhood, 0.1);
    pnh.param<bool>("icp_options/debug_print", options.optimize_options.debug_print, true);
    pnh.param<bool>("icp_options/debug_viz", options.optimize_options.debug_viz, false);
}
...
void lioOptimization::initialValue()
{
    ...
    pub_loop_map = nh.advertise<sensor_msgs::PointCloud2>(robot_id + "/sr_lio/loop_map", 2);
    // loop closing
}
...
void lioOptimization::process(std::vector<point3D> &const_frame, double timestamp_begin, double timestamp_offset)
{
    ...
    if (debug_output && index_frame % 20 == 0)
    {
    ...
    publishCurrentSampledCloud(p_frame, false);
    publishCurrentSampledCloud(p_frame, true);
...
{
    const std::string base_output_path = output_path.empty() ? "/tmp" : output_path;
    std::ofstream foutC(std::string(base_output_path + "/" + robot_id + "_pose.txt"), std::ios::app);
    ...
    if (initial_flag)
    {
        std::ofstream foutC2(std::string(base_output_path + "/" + robot_id + "_velocity.txt"), std::ios::app);
        ...
        std::ofstream foutC3(std::string(base_output_path + "/" + robot_id + "_bias.txt"), std::ios::app);
...
void lioOptimization::publishCurrentSampledCloud(cloudFrame *p_frame, bool publish_dynamic)
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZI>());
    cloud->reserve(p_frame->point_frame.size());

    for (const auto& point : p_frame->point_frame)
    {
        if (publish_dynamic != point.is_dynamic)
            continue;

        pcl::PointXYZI cloud_point;
        cloud_point.x = point.raw_point.x();
        cloud_point.y = point.raw_point.y();
        cloud_point.z = point.raw_point.z();
        cloud_point.intensity = point.label;
        cloud->push_back(cloud_point);
    }

    sensor_msgs::PointCloud2 msg;
    pcl::toROSMsg(*cloud, msg);
    msg.header.stamp = ros::Time().fromSec(p_frame->time_sweep_begin);
    msg.header.frame_id = robot_id + "/" + lidar_frame;

    if (publish_dynamic)
        pub_dynamic_points_sampled.publish(msg);
    else
        pub_static_points_sampled.publish(msg);
}
...
void lioOptimization::publish_path(ros::Publisher pub_path,cloudFrame *p_frame)
{
    ...
    msg_body_pose.header.frame_id = robot_id + "/sr_lio_init";
...
void lioOptimization::publishCLoudWorld(ros::Publisher &pub_cloud_world, pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_points, cloudFrame* p_frame)
{
    ...
    laserCloudmsg.header.frame_id = robot_id + "/sr_lio_init";
...
void lioOptimization::publish_odometry(const ros::Publisher & pubOdomAftMapped, cloudFrame *p_frame)
{
    ...
    odomAftMapped.header.frame_id = robot_id + "/sr_lio_init";
    odomAftMapped.child_frame_id = robot_id + "/sr_lio_body";
    ...
    laserOdometryTrans.frame_id_ = robot_id + "/sr_lio_init";
    laserOdometryTrans.child_frame_id_ = robot_id + "/sr_lio_laser_odom";
    ...
    for (auto &measurement : measurements)
    {
        ...
        if (!initial_flag)
        {
            ...
            if (imu_has_gravity)
                ...
                eskf_pro->forceInitWithoutGravity(imu_meas);
            imu_meas.clear();

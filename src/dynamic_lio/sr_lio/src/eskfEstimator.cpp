...
eskfEstimator::eskfEstimator()
{
    ...
    lxly = Eigen::Matrix<double, 3, 2>::Zero();
    ...
    g << 0.0, 0.0, 9.81;
    acc_cov = Eigen::Vector3d::Zero();
    gyr_cov = Eigen::Vector3d::Zero();
    acc_cov_scale = Eigen::Vector3d::Zero();
    gyr_cov_scale = Eigen::Vector3d::Zero();
    b_acc_cov = Eigen::Vector3d::Zero();
    b_gyr_cov = Eigen::Vector3d::Zero();
    ...
    num_init_meas = 1;
    gravity_constraint_enabled = true;
}
...
void eskfEstimator::forceInitWithoutGravity(const std::vector<std::pair<double, std::pair<Eigen::Vector3d, Eigen::Vector3d>>> &imu_meas)
{
    initialization(imu_meas);
    ...
    {
        initial_flag = true;
        gravity_constraint_enabled = false;
        ...
        setRotation(Eigen::Quaterniond::Identity());
        setTranslation(Eigen::Vector3d::Zero());
        ...
        ROS_INFO("IMU Initialization Done without gravity acceleration.");
        std::cout << "init_bg = " << mean_gyr.transpose() << std::endl;
    }
    else
    {
        ROS_INFO("Wait more IMU measurements for no-gravity initialization...");
    }
}
...
bool eskfEstimator::isGravityConstraintEnabled() const { return gravity_constraint_enabled; }
...
void eskfEstimator::predict(double dt_, const Eigen::Vector3d &acc_1_, const Eigen::Vector3d &gyr_1_)
{
    ...
    Eigen::Matrix<double, 3, 2> B_x = Eigen::Matrix<double, 3, 2>::Zero();
    ...
    if (gravity_constraint_enabled)
    {
        ...
        {
            F_x.block<2, 2>(15, 15) =
                -1.0 / g_norm_sq * B_x.transpose() * numType::skewSymmetric(g) * numType::skewSymmetric(g) * B_x;
        }
    }
...
{
    if (!gravity_constraint_enabled || g.norm() < 1e-6)
    {
        lxly.setZero();
        return;
    }

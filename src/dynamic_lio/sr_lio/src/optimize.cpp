...
{    
    ...
    for (int i = -1; i < max_num_iter; i++)
    {
        ...
        const bool use_gravity_constraint =
            imu_has_gravity && eskf_pro->isGravityConstraintEnabled() &&
            g_predict.norm() > 1e-6 && g.norm() > 1e-6;

        Eigen::Matrix<double, 3, 2> B_x_predict = Eigen::Matrix<double, 3, 2>::Zero();
        Eigen::Vector2d d_g = Eigen::Vector2d::Zero();
        Eigen::Matrix2d J_k_s2 = Eigen::Matrix2d::Identity();

        if (use_gravity_constraint)
        {
            Eigen::Vector3d g_predict_normalize = g_predict.normalized();
            Eigen::Vector3d g_normalize = g.normalized();
            ...
            B_x_predict = numType::derivativeS2(g_predict);
        ...
        if (converage || i == max_num_iter - 1)
        {
            ...
            if (use_gravity_constraint && g_before.norm() > 1e-6)
            {
            ...
            {
                J_k_s2 = Eigen::Matrix2d::Identity();
            }

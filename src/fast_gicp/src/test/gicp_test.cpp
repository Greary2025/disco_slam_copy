...
public:
  ...
  bool load(const std::string& data_directory) {
    ...
    // Allow overriding relative.txt via environment variable RELATIVE_TXT (e.g., set from a ROS launch file)
    std::string relative_path = data_directory + "/relative.txt";
    if (const char* env_rel = std::getenv("RELATIVE_TXT")) {
      if (env_rel[0] != '\0') {
        relative_path = env_rel;
      }
    }

    std::ifstream ifs(relative_path);
    if (!ifs) {
      return false;
    }

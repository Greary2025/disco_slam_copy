...
public:
    ...
    rclcpp::Subscription<disco_slam::msg::CloudInfo>::SharedPtr subLaserCloudInfo;
    ...
    FeatureExtraction(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
        : ParamServer("disco_slam_feature_extraction", options)
    {
        subLaserCloudInfo = this->create_subscription<disco_slam::msg::CloudInfo>(
            robot_id + "/disco_slam/deskew/cloud_info", rclcpp::SensorDataQoS(), 
            std::bind(&FeatureExtraction::laserCloudInfoHandler, this, std::placeholders::_1));

        pubLaserCloudInfo = this->create_publisher<disco_slam::msg::CloudInfo>(robot_id + "/disco_slam/feature/cloud_info", 1);
        pubCornerPoints = this->create_publisher<sensor_msgs::msg::PointCloud2>(robot_id + "/disco_slam/feature/cloud_corner", 1);
        pubSurfacePoints = this->create_publisher<sensor_msgs::msg::PointCloud2>(robot_id + "/disco_slam/feature/cloud_surface", 1);
    ...
    void publishFeatureCloud()
    {
        ...
        cloudInfo.cloud_corner  = publishCloud<PointType>(pubCornerPoints,  cornerCloud,  cloudHeader.stamp, robot_id + "/" + lidarFrame);
        cloudInfo.cloud_surface = publishCloud<PointType>(pubSurfacePoints, surfaceCloud, cloudHeader.stamp, robot_id + "/" + lidarFrame);
...
int main(int argc, char** argv)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("disco_slam_feature_extraction"), "\033[1;32m----> Feature Extraction Started.\033[0m");
